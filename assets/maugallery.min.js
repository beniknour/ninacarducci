...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},modalImg(lightboxId,direction){const activeImage=$(".lightboxImage");const activeTag=$(".tags-bar span.active-tag").data("images-toggle");const imagesCollection=[];$(".item-column").each(function(){const imgElement=$(this).children("img");if(imgElement.length&&(activeTag==="all"||imgElement.data("gallery-tag")===activeTag)){imagesCollection.push(imgElement)}});const index=imagesCollection.findIndex(img=>img.attr("src")===activeImage.attr("src"));let nextIndex;if(direction==='prev'){nextIndex=index-1;if(nextIndex<0){nextIndex=imagesCollection.length-1}}else if(direction==='next'){nextIndex=index+1;if(nextIndex>=imagesCollection.length){nextIndex=0}}
const nextImage=imagesCollection[nextIndex];activeImage.attr("src",nextImage.attr("src"))},prevImage(){var options=$.extend($.fn.mauGallery.defaults,options);$.fn.mauGallery.methods.modalImg(options.lightboxId,'prev')},nextImage(){var options=$.extend($.fn.mauGallery.defaults,options);$.fn.mauGallery.methods.modalImg(options.lightboxId,'next')},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
